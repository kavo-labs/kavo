...
{/* Generated from docs/internals/architecture/01-system-architecture.md by scripts/build-docs7.mjs. Edit that file, then run pnpm docs7:build. */}
...
| **Registry (dispatch table)** | `DefaultOperationRegistry` + `createOperationRegistry` (`core/src/operations/default-operation-registry.ts`)                                                                                                                                                                                                              | [0006](/internals/adr/0006-registry-driven-operations), [0007](/internals/adr/0007-module-augmentable-operation-metadata)           | One mechanism, several behaviors, for built-in and overridden operations; route generation reads the same table, so features get routes for free.                                                                                                                                          |
...
| **Adapter**                   | `TypeOrmRepositoryAdapter` (`typeorm/src/typeorm-repository-adapter.ts`) against core's `RepositoryAdapter`; `KavoInfrastructure` (`metadataFor` + `adapterFor`) supplies adapter _and_ metadata as one family                                                                                                            | [0001](/internals/adr/0001-clean-architecture-core-owns-contracts), [0011](/internals/adr/0011-entity-metadata-infrastructure-seam) | Core states persistence in its own vocabulary and the ORM package translates, which is what lets core keep zero runtime dependencies (ADR-0005) and stay testable with an in-memory fake.                                                                                                  |
...
| ADR                                                              | Decision                                                       |
| ---------------------------------------------------------------- | -------------------------------------------------------------- |
| [0001](/internals/adr/0001-clean-architecture-core-owns-contracts)    | Clean architecture: core owns all contracts                    |
| [0002](/internals/adr/0002-package-topology)                          | Three packages under `orms/` / `frameworks/` parents           |
| [0003](/internals/adr/0003-pnpm-plain-scripts-tsc-build)              | pnpm workspaces, plain scripts, `tsc -b` — no task runner      |
| [0004](/internals/adr/0004-lockstep-versioning)                       | Lockstep versioning                                            |
| [0005](/internals/adr/0005-core-zero-runtime-dependencies)            | Zero runtime dependencies in `@kavo/core`                      |
| [0006](/internals/adr/0006-registry-driven-operations)                | Registry-driven operation dispatch                             |
| [0007](/internals/adr/0007-module-augmentable-operation-metadata)     | Module-augmentable `OperationMetadata`                         |
| [0008](/internals/adr/0008-field-path-recursion-cap)                  | `FieldPath` recursion cap (max 5; default raised to 5 by 0051) |
| [0009](/internals/adr/0009-problem-details-error-shape)               | RFC 9457 problem details as the wire error shape               |
| [0010](/internals/adr/0010-explicit-named-barrel)                     | Explicit named barrel in core                                  |
| [0011](/internals/adr/0011-entity-metadata-infrastructure-seam)       | Entity-metadata & infrastructure seam                          |
| [0012](/internals/adr/0012-decoration-time-route-generation)          | Decoration-time route generation in `@kavo/nest`               |
| [0013](/internals/adr/0013-config-declared-soft-delete-operations)    | Soft-delete operations enabled from config, not metadata       |
| [0014](/internals/adr/0014-associate-by-id-not-deep-writes)           | Write-side relations: associate by id, no deep writes          |
| [0015](/internals/adr/0015-global-operation-defaults-are-engine-only) | Global operation defaults are engine-only, not routing         |
