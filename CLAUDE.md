...
- **OpenAPI component schemas** (`registerKavoSchemas`, `@kavo/nest`): `<Entity><Slot-in-PascalCase>` — `UserCreate`/`Update`/`Patch`/`Item`/`List`, plus `UserListItem`/`UserListMeta` for the envelope's parts, `UserPagination`/`UserInclude`/`UserSort` for the derivable query shapes, `UserFilter`/`UserQuery` for the structured filter predicate and the documented-only `filter`+`sort`+`pagination`+`fields`+`include`+`search` aggregate (ADR-0042), `User<Operation>` for a per-operation output shape, and `<Entity>ValidationError`. Shared error bodies are `KavoProblemDetails` / `KavoProblemDetailError`. These are OpenAPI `components.schemas` keys, not TS identifiers, so they deliberately drop the `Dto` suffix; the scheme is a downstream contract — a rename is a deliberate change.
...
`docs/getting-started/`, `docs/core/`, `docs/using-the-api.md`, `docs/querying/`, `docs/features/`, `docs/integrations/orms/` (per-ORM wiring), `docs/integrations/protocols/` (GraphQL, MCP), `docs/guides/` (the full `@Kavo`/`KavoModule` configuration reference), and `docs/reference/` are the adopter-facing front door; `docs/internals/` holds the design docs and ADRs ([`adr/`](docs/internals/adr/), numbered sequentially) — one ADR per load-bearing decision. `docs/internals/architecture/` mirrors the packages: one document per adapter, per protocol binding, and per engine concern (query grammar, error handling, soft delete, relations). ADRs are referenced by name in code comments; read the referenced ADR before changing the behavior it governs.
