...
describe("@Kavo custom operations (issue #145)", () => {
  ...
  it("documents a single-row response from schema.output with no x-kavo-operation-scoped when dto.output is unset (issue #467)", async () => {
    const findOneSchema = {
      safeParse: (input: unknown) => ({ success: true as const, data: input }),
      toJSONSchema: () => ({
        type: "object",
    ...
    @Kavo(Todo, { schema: { output: { item: findOneSchema } } })
    @Controller("todos")
    class UnscopedSchemaController {}

    await bootstrap(UnscopedSchemaController);
    const document = SwaggerModule.createDocument(app, new DocumentBuilder().setTitle("t").setVersion("0").build());
  ...
  it("documents a findMany list envelope from schema.output.list (issue #467)", async () => {
    ...
    @Kavo(Todo, { schema: { output: { list: findManySchema } } })
    @Controller("todos")
    class ListSchemaController {}

    await bootstrap(ListSchemaController);
    const document = SwaggerModule.createDocument(app, new DocumentBuilder().setTitle("t").setVersion("0").build());
    ...
    expect(schema).toMatchObject({
      ...
      properties: {
        items: {
          type: "array",
          items: { type: "object", properties: { id: { type: "number" }, title: { type: "string" } } },
        },
      },
    });
    expect(schema).not.toHaveProperty("x-kavo-operation-scoped");
  ...
  it("marks a schema-documented list envelope x-kavo-operation-scoped when a per-operation schema.output is set (issue #467)", async () => {
    ...
    @Controller("todos")
    class ScopedListSchemaController {}

    await bootstrap(ScopedListSchemaController);
    const document = SwaggerModule.createDocument(app, new DocumentBuilder().setTitle("t").setVersion("0").build());
    ...
    const schema = operation?.responses?.["200"]?.content?.["application/json"]?.schema;
    expect(schema).toMatchObject({ "x-kavo-operation-scoped": true, type: "object" });
  });
});
...
describe("@Kavo Swagger fallback request-body schema when no DTO is configured (issue #264)", () => {
  ...
  it("still reads as closed when a non-empty allowlist resolves to zero documentable properties", async () => {
    ...
    await withMetadata([{ name: "id", kind: "string", nullable: false, generated: true }], [], {
      schema: { input: { create: { fields: ["id"] }, update: { fields: ["id"] } } },
    });
  ...
  it("narrows the documented body to an explicit schema.input.create/update allowlist", async () => {
    @Kavo(Todo, { schema: { input: { create: { fields: ["title"] }, update: { fields: ["done"] } } } })
    @Controller("todos")
  ...
  it("types the reference-object `id` from the relation target's metadata, not the source entity's (issue #339)", async () => {
    ...
    await withDiscriminatingMetadata(relationOnlySource(), () => wordMetadata, {
      schema: { input: { create: { fields: ["word"] }, update: { fields: ["word"] } } },
    });
