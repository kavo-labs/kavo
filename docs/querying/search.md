...
Search is off by default per entity: a plain 400 if the entity hasn't turned `query.search.enabled` on. It only searches fields on the entity's `searchable` allowlist (default: every own string column). See [Allowlists](/features/allowlists).
