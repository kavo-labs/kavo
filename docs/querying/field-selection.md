...
`selectable` also decides what a response carries when no `select=` is sent. It's the one place to keep a column out of every response. See [Allowlists](/features/allowlists).
