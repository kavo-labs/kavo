...
Only fields on the entity's `filter.fields` allowlist can be filtered on. See [Allowed](/features/allowed) for how to configure that list. Filtering on anything outside it returns a 400, never a silent no-op.
