...
| `defaults`             | `DeepPartial<KavoSettings>`             | App-wide settings, one level below the built-in defaults and above every entity's own config. See [Settings](/integrations/nest/configuration/settings) for what's in `KavoSettings`.                                                                                                                                                                                                                                                                                                        |
...
`KavoContext.principal` is the authenticated caller. Kavo carries it and nothing more: core never reads or judges the value. Your own code does — a [computed field](/integrations/nest/configuration/entity-config#computed) that varies by viewer, or a replacement `OperationHandler`.
