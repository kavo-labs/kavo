...
`@kavo/nest` never built its own validation system — it just lets NestJS's `ValidationPipe` run whatever a registered `dto.create`/`update`/`patch` class is decorated with. `@kavo/next` has no decorator/DI container to hang an equivalent trick off of, so validation is a config key instead: `EntityConfig.schema`, declared on `createCrud` itself, next to `filter`/`sort`/`dto` — not on `createKavoHandler` ([ADR-0056](/internals/adr/0056-schema-config-key-and-standard-schema)).
