...
| `KAVO_QUERY_LIMIT_EXCEEDED`         | 400  | `filter.limits.maxDepth` / `filter.limits.maxInValues` exceeded                                                               |
...
| `KAVO_FORBIDDEN`                    | 403  | A resolved `policy` (operation, entity, or global scope) returned `false` for the request; also raisable by a custom handler  |
...
| `KAVO_CONFLICT`                     | 409  | Unique violation, or an FK violation blocking a delete (row still referenced by children)                                     |
...
| `KAVO_UNRESOLVED_RELATION`          | 422  | A create/update whose payload references a related row that doesn't exist (dangling foreign key)                              |
...
| `KAVO_PAGINATION_NOT_ADVANCING`     | 500  | A cursor page produced the token it was given, so following `meta.nextCursor` would loop forever (data- or adapter-dependent) |
