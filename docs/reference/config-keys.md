...
| `allowlists.selectable` | same shape                                                    | every own column + computed fields; own columns and computed-field names only — a relation-dotted entry is a bootstrap error ([ADR-0045](/internals/adr/0045-relation-projection-ceiling-removed)) |
