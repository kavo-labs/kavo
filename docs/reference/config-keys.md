...
| `allowlists.selectable` | `FieldPath<Entity,1>[] \| { exclude: FieldPath<Entity,1>[] }` | every own column; an ORM-derived field is opt-in only, via an explicit array ([ADR-0046](/internals/adr/0046-derived-fields-come-from-orm-metadata)); depth 1 — a relation-dotted entry neither type-checks nor boots ([ADR-0045](/internals/adr/0045-relation-projection-ceiling-removed)) |
...
`dto` and the per-entity form of `operations` aren't settings either. They're structural `EntityConfig` fields resolved once at `createCrud`/`@Kavo`. See [DTOs](/core/dtos) and [CRUD operations](/core/crud-operations). Virtual (ORM-derived) fields aren't `EntityConfig` at all — they're declared on the ORM entity and only their allowlist membership is configured; see [Virtual fields](/features/virtual-fields).
