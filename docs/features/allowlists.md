...
- `selectable` (`readonly FieldPath<Entity, 1>[]` \| `{ exclude: ... }`): fields usable in `select=`, and what a response carries. Capped to depth 1 — this entity's own columns only, so a relation-dotted entry (`"dictionary.id"`) neither type-checks nor boots ([ADR-0045](/internals/adr/0045-relation-projection-ceiling-removed)). An ORM-derived field ([Virtual fields](/features/virtual-fields)) is opt-in only, through an explicit array — never the `{ exclude }` form ([ADR-0046](/internals/adr/0046-derived-fields-come-from-orm-metadata)). An included relation's projection is governed by the **target** entity's own `selectable` ([ADR-0026](/internals/adr/0026-selectable-narrows-the-response-projection)); to restrict it, configure the target entity, or drop it from `includable`.
...
`{ exclude: [...] }` means "every own column except these" (for `includable`, every own relation; for `searchable`, every own string-kind column; for `creatable`/`updatable`, every relation too). An ORM-derived field is never part of that "every own column" base, for any key — `{ exclude }` can never surface one, only an explicit array can. It resolves at bootstrap against exactly the base set that key's plain default uses.
...
`selectable` takes this entity's own columns, and an opted-in ORM-derived field by explicit array, only. A relation-dotted entry (`"dictionary.id"`) is a bootstrap error ([ADR-0045](/internals/adr/0045-relation-projection-ceiling-removed)) — including in the `{ exclude }` form.
