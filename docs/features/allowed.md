...
**`create.fields`/`update.fields` only narrow — they never widen.** The array form intersects with the writable projection [`DefaultDeserializer` already derives](/internals/architecture/04-schema-system#_3-runtime-derivation-rules): naming the primary key or the soft-delete marker in an array has no effect, since neither is in that base set to begin with (naming one in an `{ exclude }` is the bootstrap error above). A registered `create`/`update`/`patch` schema with a runtime shape replaces the projection outright rather than intersecting with it — exactly as a registered `item`/`list` schema outranks `select.fields` below — so register the schema as the narrowing statement where you use one.
