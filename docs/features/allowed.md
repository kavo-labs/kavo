...
For `create.fields`/`update.fields` — as for `includable` — an `exclude` entry that names nothing in that base set (the primary key, a generated column, the soft-delete marker, a typo) is a bootstrap `ConfigurationException`, not a silent no-op: the writable base is fully known at bootstrap, so a subtraction that removes nothing is a config that can be judged wrong then and there. `{ exclude: [] }`, or any `{ exclude }` that ends up removing nothing, is exactly equivalent to omitting the key. (`filterable`/`sortable`/`selectable`/`searchable`'s own `{ exclude }` form stays lax — it silently excludes nothing on an unknown name.)
