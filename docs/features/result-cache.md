...
Entries are keyed by entity, operation, target row, app context, and query: `select`, `include`, `filter`, `sort`, `pagination`, `withDeleted`, `onlyDeleted`. `GET /users/1` and `GET /users/2` are different entries, as are `GET /users/1?select=name` and the plain read. `context.app` is in the key so one caller's values never leak to another — a custom handler or policy that varies by `context.app` is safe by construction; calls that carry no app context share one bucket. Because `context.app` is canonicalized into the key, it must be plain, shallow, JSON-serializable data with no reference cycles whenever the cache is on. A framework/ORM object passed straight through (a Passport user, a TypeORM entity, a class-transformer instance) canonicalizes by its own enumerable keys only — prototype getters are skipped — so it can hash identically for every caller and collapse them onto one bucket; a cyclic value (a request object, a logger) throws a `RangeError` at `canonicalize`'s 512-level depth guard rather than serving a wrong hit. Per-call settings are deliberately not part of the key: a per-call `delete.strategy` override that reshapes a response without changing the query is outside the cache's contract (ADR-0031).
