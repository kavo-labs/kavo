...
Whether a relation may be included at all is a separate question, answered by `allowlists.includable` (entity scope only). See [Allowlists](/features/allowlists) ([ADR-0028](/internals/adr/0028-includable-relations-move-into-allowlists)).
...
- `defaultInclude` (default `false`): includes this relation even when the client doesn't ask for it. It requires the relation to also be named in `allowlists.includable`, or it's a bootstrap error.
...
- `strategy` (default `"auto"`, or `"join"`/`"batch"`/`"key"`): controls how the relation loads. `join` runs a single query and is correct for to-one relations. `batch` runs a per-level `WHERE parentId IN (...)` and is correct for to-many relations. `auto` picks one of the two based on cardinality. `key` (to-one only) is the cheapest option when a caller wants nothing but the foreign key: it materializes the edge as `{ <pk>: value }` read straight off the parent's local FK column — no join, no extra batch query — and serializes a `null` FK as `null`. It grants no permission of its own (the edge is still includable only via `allowlists.includable`), `select[<rel>]` may name only the primary key, and it is rejected at bootstrap on a to-many edge (no local FK to read). A composite-key target is not supported yet.
...
`write` is independent of `allowlists.includable`: a relation can be write-opted without being read-includable, or the other way around.
