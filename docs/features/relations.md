...
- `strategy` (default `"auto"`, or `"join"`/`"batch"`): controls how the relation loads. `join` runs a single query and is correct for to-one relations. `batch` runs a per-level `WHERE parentId IN (...)` and is correct for to-many relations. `auto` picks one of the two based on cardinality.
