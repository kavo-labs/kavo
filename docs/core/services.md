...
`boundKavoService(this)` reaches the service `KavoModule`'s discovery binder already attached to a `@Kavo`-decorated controller instance. Both `@Override`'d methods and fully custom routes use this pattern:
