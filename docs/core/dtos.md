...
This is what keeps DTO classes plain — no decorators, no reflection library — at the cost of requiring initializers for the narrowing to actually take effect.
...
A field with no backing column — computed at serialization time from the row that was already fetched — is declared on the entity's `computed` config, not faked through a DTO class. See [Allowlists & computed fields](/features/allowlists-and-computed-fields#computed) for the full descriptor.
