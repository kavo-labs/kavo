...
The query surface Kavo can derive from the entity's resolved config is hoisted too: `AdPagination` (`{ limit, offset }` integers), `AdInclude` (an array whose items enum the top-level [includable](/features/allowlists) relation names — a nested path is dotted into one and is not enumerated), and `AdSort` (an array whose items enum the sortable keys, each present bare and `-`-prefixed for descending). These are additive documentation of the existing flat `limit`/`offset`/`include`/`sort` params — the params and the [query grammar](/querying/) are unchanged. `AdInclude` is omitted when nothing is includable; `AdPagination` is still emitted under `pagination.strategy: "none"`, carrying a description that says the entity does not paginate. All three carry `x-kavo-entity` and are produced at module init (resolved allowlists don't exist at decoration time), so an app with no `KavoModule.forRoot`/`forRootAsync` gets none of them.
