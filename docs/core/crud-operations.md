...
@Kavo(Book, {
  operations: {
    ...
    updateOne: {},
    deleteOne: {},
    // patchOne isn't named here, so it's off — no boolean needed to say so.
  },
})
```
...
**`operations` is an explicit whitelist the moment it's declared, and naming an id is what enables it.** Omitting `patchOne` above doesn't just leave it unconfigured — it disables it, because every standard operation the config doesn't name (`createOne`, `findOne`, `findMany`, `updateOne`, `deleteOne` here) has to be named to stay on, which is why they're listed as bare `{}` entries. Omit `operations` entirely and every standard operation keeps its global/built-in default; declare it and only the operations you name exist. There's no boolean shorthand and no `enabled` field — presence is the only signal. See [Guides/Configuration/Operations](/guides/configuration/operations) for the full rule.

`restoreOne`/`purgeOne` follow a different default, because there's nothing to restore or purge until soft delete is declared. `restoreOne` turns on automatically the moment an entity's `softDelete` config resolves to `"soft"` ([ADR-0013](/internals/adr/0013-config-declared-soft-delete-operations)) — but once `operations` is declared, naming it is what enables it regardless. `purgeOne` stays off until named explicitly (`operations: { purgeOne: {} }`), because permanently deleting a row is worth stating on purpose. See [Soft delete](/features/soft-delete) for the full walkthrough.
