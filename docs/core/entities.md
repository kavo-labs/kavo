...
- **At least one primary column.** A single primary column is the common case, reported as `idField`. `@kavo/typeorm` also supports a composite primary key — two or more `@PrimaryColumn`s — reported as `compositeIdFields`, an ordered list ([ADR-0039](/internals/adr/0039-composite-primary-keys-are-typeorm-only)); the other ORM adapters still require exactly one. A composite-key entity's route id is one path segment, its key columns joined by `~` (a literal `~` in a value is escaped as `~~`) — `GET /grants/u1~billing` addresses `{ userId: "u1", topic: "billing" }`. Its key columns are creatable (a natural key the client supplies on `createOne`) but not updatable (immutable afterward), cursor and since pagination aren't supported on it yet, and it can't be the target of an association-by-id write ([ADR-0014](/internals/adr/0014-associate-by-id-not-deep-writes)) — all three fail loudly at bootstrap rather than misbehave at request time.
