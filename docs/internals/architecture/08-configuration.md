...
| `relations.edges.<name>.write`                                          | unset (`false`)                                | opts a to-many relation into `arrayMutation` writes (ADR-0014's extension point, ADR-0029); rejected on a to-one relation                                                   |
| `arrayMutation.strategy`                                                | unset — no built-in default (issue #221)       | `"replace"` \| `"resource"` \| `"jsonPatch"` — all three are implemented; a write-opted relation demands one be declared; `false` disables the feature wholesale (ADR-0029) |
