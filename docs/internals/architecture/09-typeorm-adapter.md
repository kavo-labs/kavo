...
- **Includes (doc 12):** built. `buildQuery` joins to-one nodes
  ...
  `key` node (issue #364) is neither: `joinIncludes` maps its local FK id
  in via TypeORM's batched `loadRelationIdAndMap` (no join, one id-only
  query for the whole result set, no row-multiplication) and `loadBatches`
  rewrites the scratch id to `{ <pk>: value }` / `null`.
