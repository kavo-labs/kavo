...
| `KAVO_FORBIDDEN`                       | 403  | A resolved `policy` function evaluated to `false` for the current request's `context.app` (ADR-0037); available to a custom operation's own handler for the same purpose                                          | —                                 |
