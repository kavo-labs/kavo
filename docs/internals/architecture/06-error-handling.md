...
| `KAVO_HTTP_ERROR`                      | \*   | Framework-level `HttpException` reaching the filter without ever going through `KavoEngine.execute` (§6)                                                                                                                | `errors[]` when the app's `ValidationPipe` uses `kavoValidationExceptionFactory` (issue #437) |
...
- A Nest `HttpException` → `KAVO_HTTP_ERROR`, with the **response's**
  ...
  came from. An app whose `ValidationPipe` instead uses
  `kavoValidationExceptionFactory` (`@kavo/nest`, issue #437) gets one
  `errors[]` entry per **field** instead — deduped when multiple
  constraints on the same property share a message — surfaced through
  `KavoExceptionShape.issues` the same way `QueryValidationException` does.
  This is opt-in: `@kavo/nest` never installs the pipe itself (the app
  does, `examples/nest-typeorm/src/app.module.ts`), so an app that keeps
  Nest's default `exceptionFactory` sees no change.
