...
| `KAVO_ARRAY_MUTATION_INVALID_SHAPE`    | 400  | `replace<Relation>` body is not an array of `{id}` refs, or `null`; a `resource`-strategy `add`/`remove<Relation>` body is not a single id/`{id}` ref (arrayMutation's `replace`/`resource` strategies, ADR-0029)                     | —                                 |
