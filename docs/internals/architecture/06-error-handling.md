...
| `KAVO_FORBIDDEN`                       | 403  | A resolved `policy` function (operation, entity, or global scope) evaluated to `false` for the request (ADR-0037), or `authorization.required` denied one no policy covered (ADR-0035); also raisable by a custom operation's handler | —                                 |
