...
| `KAVO_ARRAY_MUTATION_INVALID_SHAPE` | 400  | `replace<Relation>` body is not an array of ids/`{id}` refs, or `null` (arrayMutation's `replace` strategy, ADR-0029)                           | —                                 |
...
| `KAVO_NOT_FOUND`                    | 404  | Target row missing on findOne/update/patch/delete; also a `jsonPatch` `add` naming an id with no matching row                                   | —                                 |
