...
| `KAVO_FORBIDDEN`                       | 403  | A configured `policy.<id>` node evaluated to `false` for the current principal (ADR-0032); available to a custom operation's own handler for the same purpose                                                         | —                                 |
