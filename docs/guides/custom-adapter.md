...
`FieldMetadata.kind` is one of `"string" | "number" | "boolean" | "date" | "enum" | "json"` — this is what the query normalizer uses to coerce a raw query-string value (`?filter[age][gte]=18`) into the right JS type before it ever reaches your adapter.
...
- **Filter/sort/pagination validation** — already done before your adapter sees the query. You only need to translate an already-valid `NormalizedQueryContext` into your backend's native query shape.
- **Soft delete's routing decision** — the engine resolves the effective strategy (soft vs. hard) from config; your adapter's `delete`/`restore`/`purge` methods just need to honor `context.config.softDelete` when deciding how to act. See [Soft delete](/features/soft-delete).
