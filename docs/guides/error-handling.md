...
`KAVO_PRECONDITION_FAILED` (412) means your `If-Match` token didn't match the row's current `ETag` — someone else wrote it since you last read it. The fix is almost always: re-`GET`, resolve the conflict, retry with the fresh tag. `KAVO_PRECONDITION_UNSUPPORTED` (also 412) is a different situation entirely — it means the check couldn't run at all (untargeted operation, caching off, `findOne` disabled), so retrying unchanged will never help. See [Caching & ETags](/features/caching-and-etags) for the full conditional-request contract.
