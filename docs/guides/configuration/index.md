...
- **[Settings](/guides/configuration/settings)**: the app-wide `KavoSettings` fields: pagination, query, errors, cache, delete, realtime.
- **[Entity config](/guides/configuration/entity-config)**: `@Kavo(Entity, config)`'s own fields: `schema`, the per-axis `filter`/`sort`/`select`/`search`/`include` blocks, plus where `policy` is set.
