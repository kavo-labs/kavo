...
[TypeORM](/integrations/orms/typeorm), [Prisma](/integrations/orms/prisma), [Mongoose](/integrations/orms/mongoose), and [MikroORM](/integrations/orms/mikroorm) cover the zero-config path. Once zero-config isn't enough, configuration resolves through one precedence chain, each scope overriding the one before it:
