...
- **[Entity config](/guides/configuration/entity-config)**: `@Kavo(Entity, config)`'s own fields: `dto`, the per-axis `filter`/`sort`/`select`/`search`/`include` blocks, plus where `policy` is set.
