...
| **Add a custom** operation               | `operations: { markPaidOne: { handler, meta: { routes } } }` (issue #145): any key outside the standard eight. `kind`/`cardinality` default to a single-row write; `schema` gives it a shape of its own. `handler` is optional (issue #424) — omit it only when an `@Override` method supplies the implementation (see below); otherwise it is required. |
